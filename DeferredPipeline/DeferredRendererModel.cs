...
namespace DeferredPipeline
{
		...
		new string[]
	{
		...
		private void LookUpTextures(NodeContent node)
		{
			...
			if (mesh != null)
			{
				...
				else
				{
					...
					object value;
					mesh.OpaqueData.TryGetValue(NormalMapKey, out value);
					normalMapPath = (string)value;
				}
				...
				else
				{
					...
					object value;
					mesh.OpaqueData.TryGetValue(specularMapKey, out value);
					specularMapPath = (string)value;
				}
		...
		{
			GBufferEffectMaterialContent materialContent = new GBufferEffectMaterialContent();
			...
			foreach (KeyValuePair<String, ExternalReference<TextureContent>> texture in material.Textures)
			{
				if ((texture.Key == "Texture") ||
					...
					materialContent.Textures.Add(texture.Key, texture.Value);
			}

			return context.Convert<MaterialContent, MaterialContent>(materialContent, typeof(MaterialProcessor).Name);
		}
	}
}